...
public abstract class Solver
{
    ...
    private LinkedList scc; // List of strongly connected components that
                            // are left to work on
    private Collection currentSCC; // Set of equations that is the current strongly
                            // connected component that is being worked on
    private Collection equations; // Set of all equations

    /** 
     *  Map from variables to Set of equations containing them. If the bound 
     * of the variable v changes, then we may need to re-examine all equations
     * in (Set)varEqnDependencies.get(v)
     */
    private Map varEqnDependencies; 
    
    /** 
     *  Map from equations to Set of variables whose bound may be modified as 
     * a result of solving that equation. If the bound of
     * variables in the equation eqn are modified, then the variables will
     * be contained in (Set)eqnVarDependencies.get(eqn)
     */
     ...
     * variables. This
     * map allows us to detect early if some constraint violates an
     * equality constraint. Note that early checking of equality constraints
     ...
     *
     * If true, the constraints are partitioned into strongly
     * connected components, which are then sorted in topological
     * order. If false, then constraints are solved in the order they
    ...
    /**
     * Convenience method to determine if messages of the given obscurity 
     * should be reported.
     */
     ...
     * 
     * XXX Maybe do most of this work while adding constraints?
     */
    protected final Graph createGraph() {
        ...
        for (Iterator e = nodes.iterator(); e.hasNext();) {
            ...
            else if (!toCheck.env().hasVariables() && !lhs.hasVariables() && !rhs.hasVariables()) {
                removedEqns.add(toCheck);
                e.remove();
                if (shouldReport(4)) report(4, toCheck.toString());
            }
...
/*        if (removedEqns.size() + nodes.size() != equations.size()) {
     ...
     * @param responsible the constraint that was responsible for modifying the bound.
     * @throws SemanticException if the new bound violates an equality constraint.
     */
    ...
    throws SemanticException {
        ...
        if (eqConstraints != null) {
            ...
            for (Iterator iter = eqConstraints.iterator(); iter.hasNext(); ) {
                ...
                if (!(eqCnstr.env().leq(newBound, boundRHS) &&
                      eqCnstr.env().leq(boundRHS, newBound))) {
    ...
    /**
     * This method must return a constant label, which is the default bound
     * of variables.
     */
    protected abstract Label getDefaultBound();

    /**
     * Solve the system of constraints, by finding upper bounds for the 
     * label variables.
     * 
     * @return a solution to the system of constraints, in the form of a 
     *         VarMap of the upper bounds of the label variables.
     * @throws SemanticException if the Solver cannot find a solution to the
     *         system of contraints.
     */
    protected VarMap solve_bounds() throws SemanticException
    {
        ...
        while (! Q.isEmpty()) {
            ...
            if (eqn.env().leq(lhsbound, rhsbound)) {
                if (shouldReport(4))
                report(4, "constraint: " + eqn + " already satisfied: " +
                lhsbound + "<=" + rhsbound);
            } 
            else {            
                if (shouldReport(3)) {
                report(3, "Considering constraint: " + eqn 
                    + " (line " + eqn.position().line()+ ")");
                }
                // let the subclass deal with changing the bounds on variables
    ...
    protected final void wakeUp(VarLabel v)
    {
        ...
        if (eqns != null) {
            ...
            if (!Q.contains(eqn) && (currentSCC == null || 
                        currentSCC.contains(eqn))) // offensive but simple
                Q.add(eqn);
            }
        }
    }
    ...
    /**
     * Constraint number at which to stop the compiler, for debugging
     * purposes. Set using a command line option, refer to 
     * {@link jif.ExtensionInfo} for details
     */
    ...
    protected final void inc_counter() {
        ...
        if (constraint_counter == stop_constraint) {
            ...
            throw new RuntimeException("Halting at constraint " + stop_constraint);
        }
    }
    ...
    public final void addConstraint(LabelConstraint c) throws SemanticException {
        ...
        if (shouldReport(4))
            report(4, (constraint_counter+1) + ": " + c);
        if (shouldReport(5))
            report(5, ">>> " + c.msg());
        inc_counter();
        ...
        // XXX changed this not-taken logic -- correct now?
        ...
        if (c.rhs() instanceof NotTaken && c.kind() == LabelConstraint.LEQ) {
            // if the RHS is NotTaken, then the constraint can
            // never be satisfied.
            throw new SemanticException(errorMsg(c), c.position());
        }
        ...
        for (Iterator i = eqns.iterator(); i.hasNext(); ) {
            eqn = (Equation)i.next();
            if (!eqn.env().hasVariables() && 
                !eqn.lhs().hasVariables() && 
                ...
                if (! eqn.evaluate()) {
                    ...
                    throw new SemanticException(errorMsg(eqn.constraint()), eqn.position());
                }
                else {
    ...
    /**
     * This abstract method must add the correct dependencies from Equation
     ...
     * var may be modified as a result of solving eqn. This dependency should 
     * be recorded by calling the method addDependency(eqn, var).
     * 
    ...
    /**
     * Returns the equations that are dependent on the equation eqn
     * by finding the variables that eqn may alter if it is solved (useing the 
     * map eqnVarDependencies), and then finds the equations that depend on 
     * those variables (using the map varEqnDependencies)
     */
...
//    }
    ...
    /**
     * Record the fact that label variable v, in the constraint
     * eqn had its bound set to the label lb.
     */
    ...
    /**
     * @return  an equation that contained label variable var, and
     * that changed the bound of var to less than/greater than threshold. 
     * Null if no such equation exists. 
     * If lowerThreshold is true, then the equation returned is one that
     * changed var to less than threshold; otherwise the equation returned
     * is one that changed var to greater than the threshold
     */
    protected final Equation findTrace(VarLabel var, Label threshold, boolean lowerThreshold) {
        ...
        if (history != null) {
            for (Iterator iter = history.iterator(); iter.hasNext(); ) {
                ...
                boolean test = lowerThreshold ? 
                                    eqn.env().leq(threshold, label) : 
    ...
    /**
     * Report the traces for each variables in the collection <code>Variables</code>
     */
            ...
            for (Iterator vs = variables.iterator(); vs.hasNext(); ) {
                ...
                if (trace != null) {
                    ...
                    for (int i = 0; i < trace.size(); ++i) {
                        ...
                        trc.append("  " + i + ": " + l + " from eqn " + e.lhs() + " <= " + e.rhs() + "\n");
                        trc.append("       from " + e.constraint());
                        if (e.constraint().position() != null) {
                            trc.append(" (line " + e.constraint().position().line() + ")");
                        }
                        trc.append("\n");
                    }                    
    ...
    throws SemanticException 
    {
        while (!c.report()) {
            ...
            if (shouldReport(3)) 
                report(3, "Found contradictive eqn for " + c + "; it is " + eqn);
            c = eqn.constraint();
        }
