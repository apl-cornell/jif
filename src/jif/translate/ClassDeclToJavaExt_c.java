...
public class ClassDeclToJavaExt_c extends ToJavaExt_c {
    ...
    public Node toJava(JifToJavaRewriter rw) throws SemanticException {
        ...
        if (!jpt.flags().isInterface()) {
            if (!rw.jif_ts().isSignature(jpt)) {
                ...
                if (hasDefaultConstructor) {
                    cb =
                            cb.addMember(produceDefaultConstructorInvoker(jpt,
                                    rw, defaultConstructorExceptions));
                }
                ...
                if (!jpt.params().isEmpty()) {
                    ...
                    if (!rw.jif_ts().isSignature(jpt)) {
                        for (ParamInstance pi : jpt.params()) {
                            ...
                            cb =
                                    cb.addMember(rw.qq().parseMember(
                                            "private final %T %s;", tn,
                                            paramFieldName));
                        }
                    }
                }
            ...
            if (rw.jif_ts().needsImplClass(jpt)) {
                ...
                implBody =
                        implBody.addMember(produceInstanceOfMethod(jpt, rw,
                                true));
                implBody = implBody.addMember(produceCastMethod(jpt, rw));

                Id name =
                        rw.java_nf().Id(Position.compilerGenerated(),
                                interfaceClassImplName(n.name()));
                ClassDecl implDecl =
                        rw.java_nf().ClassDecl(Position.compilerGenerated(),
                                n.flags().clearInterface().Abstract(), name,
                ...
                for (ParamInstance pi : jpt.params()) {
                    ...
                    TypeNode tn = typeNodeForParam(pi, rw);
                    cb =
                            cb.addMember(rw.qq().parseMember("%T %s();", tn,
        ...
        if (n.type().superType() instanceof JifSubstClassType_c) {
            ...
            for (ParamInstance pi : ((JifParsedPolyType) superType.base())
                    .params()) {
                if (pi.isType()) {
                    TypeParam tp =
                            (TypeParam) ((JifSubst) superType.subst()).get(pi);
        ...
        for (int i = 0; i < n.interfaces().size(); i++) {
            ...
            if (n.type().interfaces().get(i) instanceof JifSubstClassType_c) {
                ...
                List<TypeNode> args = new ArrayList<TypeNode>();
                for (ParamInstance pi : ((JifParsedPolyType) interType.base())
                        .params()) {
                    if (pi.isType()) {
                        TypeParam tp =
                                (TypeParam) ((JifSubst) interType.subst())
                                        .get(pi);
                        Named namedType = (Named) tp.type();
                        args.add(nf.AmbTypeNode(tp.position(),
                                nf.Id(tp.position(), namedType.name())));
    ...
    protected ClassBody addStaticInitializers(ClassBody cb, JifToJavaRewriter rw) {
        ...
        return cb.addMember(rw.java_nf().Initializer(
                Position.compilerGenerated(), Flags.STATIC, b));
    }
